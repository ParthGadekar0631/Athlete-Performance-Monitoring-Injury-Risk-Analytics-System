...
async function loadStaticCsvData() {
  const response = await fetch("dashboard_export.csv");
  if (!response.ok) throw new Error(`Static dashboard data failed: ${response.status}`);
  ...
  return {
    records,
    players: Array.from(playerMap.values()).sort((a, b) => a.player_name.localeCompare(b.player_name)),
    teams: [...new Set(records.map((row) => row.team).filter(Boolean))].sort(),
    positions: [...new Set(records.map((row) => row.position).filter(Boolean))].sort(),
